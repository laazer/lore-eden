...
import type { LayoutEdit } from "../layout/store";
import {
  DEFAULT_ITEM_HEIGHT,
  DEFAULT_ITEM_WIDTH,
  addItem,
...
import type { ViewLayout, ViewViewport, ViewViewportPatch } from "../layout/types";
import { CanvasItemView, type CanvasActions } from "./CanvasItemView";
import type { RenderPrimitivePicker } from "../panes/ContainerPane";
import type { PrimitiveRegistry } from "../panes/primitives/registry";
import { ICON_BUTTON } from "../panes/paneChrome";
import "../panes/paneChrome.css";
 ...
 * the host, and what is passed here is only what to draw.
 *
...
export function CanvasSurface({
  ...
  /**
   * Queue an arrangement edit. The host owns persistence — see `layout/store`.
   */
  edit: (edit: LayoutEdit, onSettled?: () => void) => void;
  /**
   * Persist pan/zoom.
   *
   * A separate path from `edit` on purpose, and not a tidiness preference: pan
   * and zoom fire continuously while a content edit may be in flight, and
   * queueing them behind it makes the canvas feel stuck. They are also
   * independent records, so there is nothing to serialize between them.
   */
  writeViewport: (patch: ViewViewportPatch) => void;
  /** Store a container's chosen primitive. */
  pickPrimitive: (containerId: string, primitiveId: string) => void;
  /**
   * Identifies this surface in the DOM, for a host's own tests and tooling.
   *
   * A seam rather than a nicety: the suites that pin canvas behaviour select by
   * it. Empty when a host has nothing to call it.
   */
  surfaceId?: string;
  /** The picker a pane opens when it has no primitive yet. */
  renderPicker?: RenderPrimitivePicker;
  /** Which primitives this surface offers. Defaults to the shared registry. */
  ...
  // The header's picker and the empty-pane picker are the same slot, reached
  // through `actions` rather than obtained separately: no primitive *component*
  // is imported here, which is the invariant that lets a host add one without
  // touching this file.
  ...
  const actions = useMemo<CanvasActions>(
    ...
    [edit, pickPrimitive, renderPicker, registry],
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", flex: "1 1 0", minHeight: 0 }}>
        <div
          ref={viewport}
          data-canvas-viewport={surfaceId}
