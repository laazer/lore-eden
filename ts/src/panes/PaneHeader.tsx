...
/** What the settings control is called, in both its accessible name and its title. */
...
export function PaneHeader({
  ...
  drag?: PaneDragHandle;
  /**
   * The settings panel, when the settings control is activated.
   *
   * A slot rather than a component imported here: a settings editor is built
   * from the host's own form controls, and the header has no business choosing
   * them. Omitted, the settings control does not appear at all — a button that
   * opens nothing is the control-that-lies this component already refuses for
   * a primitive with no fields.
   */
  ...
  /**
   * The primitive picker, when the picker control is activated.
   *
   * Also a slot, and for a second reason beyond styling: which primitives are
   * offered is the host's decision, not this package's.
   */
  renderPicker?: (props: {
    onClose: () => void;
  ...
  const configurable =
    ...
    primitive !== undefined &&
    primitive.settingsFields.length > 0;
