...
def dynamic_access_errors(
    ...
    escape hatch, which left reaching for an optional attribute on a foreign
    object — a stdlib AST node that may not carry ``end_lineno`` — with no
    answer except disabling the gate wholesale.
    """
    ...
    for node in ast.walk(tree):
        ...
        if lineno is None or not _span_touched(lineno, lineno, touched_lines):
            continue
        if _line_waives(content_lines, lineno, _ALLOW_DYNAMIC):
            continue
        errors.append(
...
def _is_ast_node_type(target: str) -> bool:
    """True for ``ast.Call`` and friends — stdlib AST node classes.

    Deliberately textual: the check runs on unparsed source, and a module the
    file aliased (``import ast as _ast``) is rare enough that missing it costs
    only a finding the author can waive.
    """
...
def isinstance_errors(
    ...
    dispatch on. Type-testing nodes *is* the visitor idiom the language offers,
    so flagging it produced a finding with no fix on every AST walker — the
    waiver comment would be the only possible response, on every line, forever.
    """
    if _is_test_path(py_file):
        return []
...
def check_file(
    ...
    *,
    ...
    errors.extend(dynamic_access_errors(py_file, tree, content.splitlines(), touched_lines))
    errors.extend(isinstance_errors(py_file, tree, content.splitlines(), touched_lines))
    errors.extend(mid_dot_fstring_errors(py_file, tree, touched_lines, house_rules))
    errors.extend(
...
def _joined_str_has_mid_dot(node: ast.JoinedStr) -> bool:
    ...
    for value in node.values:
        if isinstance(value, ast.Constant) and isinstance(value.value, str) and _MID_DOT in value.value:  # py-org: allow-isinstance (ast node)
            return True
    return False
...
def mid_dot_fstring_errors(
    ...
    staged lines. A joining helper builds its label via ``" · ".join(...)`` (a
    Call on a Constant), so it does not self-trigger.

    Off unless the repo names its helper in ``.lore-eden-gates.json`` — telling
    someone to use a helper without saying which one is not an actionable
    finding. See :mod:`house_rules`.
    """
    if not house_rules.mid_dot_enabled:
        return []
    if _is_test_path(py_file):
        return []
    ...
    for node in tree.body:
        ...
        for fn in funcs:
            ...
            errors.append(
                ...
                f"`{house_rules.mid_dot_helper}` instead of f-strings with ` · `"
            )
    return errors
...
def _source_segment_from_lines(lines: List[str], node: ast.AST) -> Optional[str]:
    ...
    # Optional attributes on a stdlib AST node, which does not always carry
    # them; there is no typed accessor to reach for.
    end_lineno = getattr(node, "end_lineno", None)  # py-org: allow-dynamic
    end_col_offset = getattr(node, "end_col_offset", None)  # py-org: allow-dynamic
