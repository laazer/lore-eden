...
from house_rules import HouseRulesError, load_house_rules  # noqa: E402 - path set above
...
def render_block(
    ...
    ``excluded`` names gates this repo has opted out of, with the reason. They
    are omitted from the block *and* their reason written into it, because the
    next person to read this file needs to know a gate is missing on purpose
    rather than by an install that went wrong.

    The exclusions live in the target repo's `.lore-eden-gates.json` rather than
    in a flag, because this block is regenerated on every install: a hand-edit,
    or a one-off `--exclude`, would be silently undone by the next refresh.
    """
    ...
    for name, reason in sorted(excluded.items()):
        body.append(f"# Not installed — {name}: {reason}")
    for gate in MANAGED_GATES:
        if gate.name in excluded:
            continue
        body.extend(
            [
                f"{gate.name}:",
...
def main() -> int:
    ...
    try:
        excluded = load_house_rules(config.parent).excluded_gates
    except HouseRulesError as exc:
        # A malformed config is not "no exclusions". Installing the full block
        # over a repo that asked for a subset is the double-gating this is for.
        print(f"skip: {exc}", file=sys.stderr)
        return 1
    unknown = sorted(set(excluded) - set(MANAGED_COMMAND_NAMES))
    if unknown:
        print(
            f"skip: {config.parent} excludes unknown gate(s) {', '.join(unknown)}; "
            f"known names are {', '.join(MANAGED_COMMAND_NAMES)}",
            file=sys.stderr,
        )
        return 1
    block = render_block(
        Path(args.gates_root), entry_indent, repo_root=config.parent, excluded=excluded
    )
